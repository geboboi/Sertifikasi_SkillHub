...
@section('title', $class->name . ' - SkillHub')
    ...
    <div class="px-6 py-5">
        <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
            <div>
                ...
                <dd class="mt-1 text-sm text-gray-900">{{ $class->instructor }}</dd>
            </div>
            
            <div>
                <dt class="text-sm font-medium text-gray-500">Total Participants</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $class->participants->count() }} students</dd>
            </div>
            
            <div class="sm:col-span-2">
                <dt class="text-sm font-medium text-gray-500">Description</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $class->description ?? 'No description provided' }}</dd>
            </div>
            
            <div>
                <dt class="text-sm font-medium text-gray-500">Created Date</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $class->created_at->format('F d, Y') }}</dd>
            </div>
        </dl>
    </div>
</div>
...
<div class="mt-8">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold text-gray-900">Enrolled Participants ({{ $class->participants->count() }})</h2>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-500">{{ \Carbon\Carbon::parse($participant->pivot->enrolled_at)->format('M d, Y') }}</div>
                    </td>
